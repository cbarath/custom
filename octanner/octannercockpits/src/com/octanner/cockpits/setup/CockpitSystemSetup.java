...
import com.octanner.cockpits.constants.OctannerCockpitsConstants;
 ...
 */
@SystemSetup(extension = OctannerCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(OctannerCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(OctannerCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						OctannerCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/octannercockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
