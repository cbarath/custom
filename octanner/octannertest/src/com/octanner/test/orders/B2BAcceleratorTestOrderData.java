...
import com.octanner.test.constants.OctannerTestConstants;
...
public class B2BAcceleratorTestOrderData
{
	...
	public void createSampleOrders()
	{
		...
		placeOrderWithInCustomerPermissions(OctannerTestConstants.PAY_USER_UID, "ORDER_SENT_NOTIFICATION_SENT paid", false);

		placeOrderRequiringApproval(OctannerTestConstants.HISTORICAL_USER_UID, "PENDING_APPROVAL");
		placeOrderForQuoteNegotiation(OctannerTestConstants.HISTORICAL_USER_UID, "PENDING_QUOTE");
		placeOrderAndApproveByB2BApprover(OctannerTestConstants.HISTORICAL_USER_UID, "ORDER_SENT_NOTIFICATION_SENT Approved");
		placeOrderWithInCustomerPermissions(OctannerTestConstants.HISTORICAL_USER_UID,
				"ORDER_SENT_NOTIFICATION_SENT costcenter", true);

		placeReplenishmentOrderInCancelledState(OctannerTestConstants.REPENISHMENT_USER_UID, "REPLENISHMENT_ENDED");
		placeReplenishmentOrderInActiveState(OctannerTestConstants.REPENISHMENT_USER_UID, "REPLENISHMENT_ONGOING");
		placeReplenishmentOrderScheduleForFuture(OctannerTestConstants.REPENISHMENT_USER_UID, "REPLENISHMENT_FUTURE");

		placeOrderQuoteRequestWithMerchantResponse(OctannerTestConstants.QUOTE_USER_UID, "APPROVED_QUOTE");
		placeOrderQuoteRequestWithMerchantResponse(OctannerTestConstants.QUOTE_USER_UID, "REJECTED_QUOTE");
		placeOrderExceedingCreditLimit(OctannerTestConstants.QUOTE_USER_UID, "PENDING_MERCHANT_APPROVAL");
		placeOrderExceedingCreditAndRejected(OctannerTestConstants.QUOTE_USER_UID, "REJECTED_BY_MERCHANT");
	...
	{
		createPaymentInfo(OctannerTestConstants.PAY_USER_UID, "USD", createVisaCardInfo(), createUsBillingInfo());
		createPaymentInfo(OctannerTestConstants.PAY_USER_UID, "USD", createMasterCardInfo(), createGermanyBillingInfo());
	}
			...
			final boolean useCostCenter)
	{
		...
		try
		{
			...
			final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					useCostCenter ? OctannerTestConstants.STANDARD_COSTCENTER : null, null);
	...
	protected String placeOrderForQuoteNegotiation(final String userID, final String purchaseOrderNumber)
	{
		...
		try
		{
			...
			final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					OctannerTestConstants.STANDARD_COSTCENTER, "Please approve request");
	...
	protected String placeOrderExceedingCreditAndRejected(final String userID, final String purchaseOrderNumber)
	{
		...
		try
		{
			...
			final OrderData orderData = placeOrder(userID, products, createUsAddressData(), purchaseOrderNumber,
					OctannerTestConstants.LOWLIMIT_COSTCENTER, null);
	...
	{
		// Lookup the site
		final CMSSiteModel cmsSite = getCmsAdminSiteService().getSiteForId(OctannerTestConstants.POWERTOOLS_SITE);
		// Lookup the customer
			...
			final B2BCustomerModel customer)
	{
		LOG.info(String.format("Creating order for [%s] for site [%s]", customer.getUid(),
				OctannerTestConstants.POWERTOOLS_SITE));
		...
		if (containsOrder(orderList, purchaseOrderNumber))
		{
			LOG.info(String.format("Not creating order because purchaseOrderNumber %s for [%s] for site [%s] already exists. ",
					purchaseOrderNumber, customer.getUid(), OctannerTestConstants.POWERTOOLS_SITE));
	...
	{
		final CMSSiteModel cmsSite = getCmsAdminSiteService().getSiteForId(OctannerTestConstants.POWERTOOLS_SITE);
		final B2BCustomerModel customer = getUserService().getUserForUID(OctannerTestConstants.APPROVER_UID.toLowerCase(),
